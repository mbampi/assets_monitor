...
class AssetMonitoringList(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        for monitored_asset in monitored_assets:
            try:
                quotation = Quotation.objects.filter(asset=monitored_asset.asset).order_by('-date', '-time')[0]
                logging.info(quotation)
                monitored_asset.last_price = quotation.price
            except IndexError:
                monitored_asset.last_price = None
...
class AssetDetail(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if price is None:
                return Response({'error': 'Error fetching price'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            else:
                return Response({'symbol': asset.symbol, 'name': asset.name, 'price': price})
        ...
        try:
            ...
            if enable:
                ...
                try:
                    ...
                    return HttpResponseBadRequest('Invalid value for lower_tunnel or upper_tunnel or frequency')
        ...
        if symbol is not None:
            ...
            if n is not None:
                try:
                    ...
                    return Quotation.objects.filter(asset__symbol=symbol).order_by('-date', '-time')[:n]
            else:
